"""Regression tests for stock_analyzer/db.py::load_judgment_opinions()
...
def test_real_rows_returned(monkeypatch):
    rows = [{"source": "risk_advisor", "dimension": "beta", "ticker": "_PORTFOLIO",
             "signal_date": "2026-08-01", "opinion_signal": 0.5}]
...
def test_paginates_past_page_size(monkeypatch):
    ...
    monkeypatch.setattr(db, "_JUDGMENT_OPINIONS_PAGE_SIZE", 2)
    rows = [
