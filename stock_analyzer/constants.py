...
INVESTIGATOR_LLM_TIMEOUT_SECONDS = 90  # per-call network timeout for the Investigator's plan/report LLM calls (2026-09-18, live production finding, same session as the token bump above) — the larger report token budget gives a thinking-heavy model more room to actually USE, which made the OLD 30s default (ai_provider.call_llm's own generic fallback) genuinely too short: a real report call hit anthropic's APITimeoutError before finishing. Passed explicitly by the Investigator's llm_fn closure — never touches ai_provider.call_llm's own default, which stays general-purpose for any future caller
