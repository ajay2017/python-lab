...
- **Buy Candidates dual-verdict divergence** — **FOUND 2026-07-27, not yet decided.** `daily_briefing._cross_reference()` returns two independently-computed verdicts (legacy `verdict`, which drives sorting/coloring across most of `app.py`, vs. `verdict_reconciled` from `signal_reconciliation.reconcile_signals()`, whose one-liner is the prominent explanation text) that can disagree — pinned by a regression test, not yet confirmed as user-observed. Needs a product decision (teach `reconcile_signals` about analyst revisions / retire the legacy field / leave as-is) before any fix — Opus-review territory since it changes a recommendation surface. Memory `project_verdict_divergence`.
