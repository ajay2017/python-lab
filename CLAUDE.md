...
- **Thesis Red Team Agent Phase 3** (F-196) — Daily Brief "Thesis Under Pressure" annotation: flags a ticker when `erosion_score >= THESIS_EROSION_BRIEF_MIN` (50) and yesterday's score was below that, OR a single-day jump `>= THESIS_EROSION_BRIEF_JUMP` (15) regardless of absolute level. Both constants already staged in `constants.py` (unused elsewhere). Phase 2 (Haiku counter-evidence + Pre-Mortem loop) shipped 2026-07-27, commit `78a6adc`. **Not gated on a date or data trigger** — genuinely just not started yet; needs its own Opus plan review before build. `docs/plans/thesis-red-team-agent.md:327-337`; memory `project_agentic_intelligence_roadmap`.
- **Entry Timing Tab Phase 2** (F-220) — a live caution annotation directly on the Grow Today pick card at the moment a `new_pick` fires (same pattern as Analyst Coverage's Grow Today caption / Day Shock's Home banner) — the only mechanism with a plausible causal path to actually preventing a loss, since Phase 1's retrospective tab (⏱️ Entry Timing, 📊 Predictive Analytics) requires the user to remember to check it. **Gated explicitly on Phase 1 validating the divergence pattern holds over a real deduped sample — not on a date.** Even once built, stays a caution, never a block. **Phase 1 SHIPPED 2026-07-28** (commits `34f4782`, `eea519e`, `29540d1` — see `docs/shipped-log.md`). `docs/plans/entry-timing-tab.md` (Phase 2 section); memory `project_entry_timing_tab`.
