...
- **Account-baseline foundation (own 💰 Account page; `stock_analyzer/account.py` + `account_cash`/`account_flows` tables; plan docs/plans/account-baseline.md):** v1 total value + cash% + true concentration (467b431/069b199), v2 flows ledger + growth-vs-contributed-capital (15b87b1), v3 money-weighted (Modified Dietz) return + annualized (0488cdc), v4 signed net cash = margin/liability awareness (f0abdf7). Reconciles to the user's Robinhood. Reuses the same tables the Robinhood MCP sync would auto-fill. **Gate-basis decision RESOLVED & SHIPPED 2026-06-26 (do NOT call it deferred/equity-weight):** ALL concentration gates (Grow Today 15%/35% suppression) AND recs (risk_advisor sector/single-name trims, entry nudge, watchlist/quick-research/comparison) use the **margin-aware tighter-of-both** basis = MV ÷ `min(equity, net capital)` via `concentration.gating_denominator` (margin tightens; cash on hand never loosens). Beta/Sharpe recs stay equity-basis. Gate and rec agree — no basis conflict. See `docs/plans/account-baseline.md` (resolution §), reqs G-19, `project_concentration_discipline`. User Guide + architecture §6.7/6.8 documented.
