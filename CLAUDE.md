...
- **Sector diversification — true benchmark-relative view** — **PARKED 2026-07-30, not started.** F-222 (Sector Gaps pointer + widened `_DIVERSIFYING_SECTORS` coverage, `docs/shipped-log.md`) shipped the "which sectors do we hold 0% of, among ones we already consider genuine diversifiers" view. A separate, bigger idea was explicitly discussed and deferred: comparing portfolio sector % against an actual S&P 500/GICS benchmark sector-weight table — no such benchmark data exists anywhere in the codebase today, and picking which benchmark to use is its own policy decision. Don't re-propose building this without the user re-raising it. Memory `project_sector_diversification_gaps`.
