...
- **Structural Vulnerability Scanner Phase 2** (F-198) — a Home-page "Structural alert" banner for a newly-formed risk cluster since yesterday. Phase 1 (Blast Radius Map + daily Haiku narrative, 🧬 Structural Scan tab) shipped 2026-07-24. **Trigger:** ≥3 days production observation — check current status before starting, this gate is close to clearing (or may have already). `structural_scanner.py`; memory `project_agentic_intelligence_roadmap`.
- **Multi-Agent Debate Phase 3** (F-197) — an AI Insights debate log (a browsable history of past entry/exit debate verdicts). Phase 1 (entry debates) shipped 2026-07-23; Phase 2 (exit debates / "Challenge This Exit") shipped 2026-07-24 — **Phase 3 is the only part of this one still open.** No fixed trigger date, not yet scoped in detail — "follows Phase 2" per the original plan. `debate_agent.py`; memory `project_agentic_intelligence_roadmap`.
- **Summary page — cross-page pointer cards** — **card #1 (Risk Posture) SHIPPED 2026-07-27** via a concurrent session's separately-scoped Tier-3 KPI-row work (plain pointer badge, no correlated-pairs flag) — reconciled cross-session same day, don't rebuild it. **Card #2 (🩺 Thesis Review) SHIPPED 2026-07-27** — reads `thesis_reviews` (F-1's per-holding review), points to 🧠 AI Insights; corrects a naming/destination mistake from this item's own origin (was mislabeled "Thesis Red Team" pointing at the ⚠️ Red Team tab, a different feature/table entirely — see `docs/plans/summary-page-pointer-cards.md` for the full correction). **Card #3 (🔔 Catalyst Watch) SHIPPED 2026-07-27** — count of held names reporting earnings within `CATALYST_WATCH_WINDOW_DAYS` (7d), sharing the SAME 24h `@st.cache_data` cache (`_cached_held_earnings_dates()`) Catalyst Watch's own page uses, verified not assumed. **Card #4 PLANNED, not started:** Watchlist actionable count (Research group) — the last one, and still the least certain re: reuse-vs-recompute. Full spec, reuse-not-recompute rules, and per-card verification notes in `docs/plans/summary-page-pointer-cards.md`. Signal Coherence and a Recommendations-engine snapshot card were considered and deliberately held back (nav-group collision), not forgotten — don't re-propose them as new ideas. memory `project_summary_page`.
