...
**🧾 Summary** (F-204) — *The at-a-glance dashboard.* KPI tiles, Act Today, holdings table (moved here from Home, F-01), plus pointer cards: Engine Track Record (F-229), State of the Portfolio standing thesis (F-232). When a live EXIT/TRIM item is present and the engine's own protective-call track record is measurably running early, Act Today shows a factual disclosure of that pattern (F-277, 2026-09-20) — never a reason by itself to override the call. Look for a fast status read without the full Home synthesis.

**💰 Account** — *Cash, account value, capital trend, and reports.* Account-baseline: cash + total account value (F-03b), a flows ledger for growth-vs-contributions (F-03c), and a Capital Trend chart with a net-capital-contributed overlay (F-03d). Leverage/margin awareness (F-09d) lives here too — **read-only, never gates.** Three tabs: 💰 Account, 🔌 Brokerage Trend, and 📄 Reports (owner-only). The Reports tab hosts two report types via a radio selector: a **Tax Report** (F-276 — FIFO realized short-term/long-term gains by tax year, wash-sale flags, CSV/markdown export, explicitly **not tax advice**) and a **Performance Review** (F-276b — a point-in-time snapshot for a picked date range: return vs SPY, recommendations acted/skipped, gates fired, trade behavior, leverage/risk drift; a period below the standalone pages' evaluable floor shows raw counts only, never a contradicting verdict). Look when reconciling real account value, checking margin, or pulling a tax-year gains summary.
...
*Added 2026-09-24 (app review Q4) — collects the app's retrospective/measurement-only surfaces out of RESEARCH and PORTFOLIO into one home. Navigation regrouping only: no page's own content, tabs, gates, or scoring changed.*
...
**🔬 Model Lab** (F-234) — *Owner-only, experimental.* Quarantined forward-volatility shadow layer: 20-day EWMA vol forecast per held ticker + portfolio, scored against a naive persistence baseline. Feeds no gate, no recommendation, no composite. Dead-end by design — a measurement harness to validate before any signal is ever wired in. Grouped here despite being forward-forecasting, not backward-grading — same "feeds nothing live, quarantined validation harness" DNA as the rest of this group, a better fit than sitting beside RESEARCH's live decision tools.
...
**🧠 AI Insights** — *The LLM advisory layer.* Eight tabs: 🩺 Positions (thesis review INTACT/WEAKENING/BROKEN, F-151; behavioral KPI strip F-166) | 📅 Debriefs (weekly F-152) | 🏦 Research (Ideas Inbox F-154) | 📊 Scorecard (analyst call accuracy F-154c — KPI row, per-call table, firm leaderboard, best/worst calls, plus a **Phase 3 "⚖️ Engine vs Analyst Calibration" 2×2** showing whether saved analyst consensus or the engine composite called it right when the two disagreed at save time) | ⚠️ Red Team (thesis counter-evidence, F-196) | ⚔️ Debate Log (multi-agent debate, F-197) | 💬 Ask (Portfolio Q&A, F-225) | 🔎 Investigator (owner-only, F-275 — a different kind of tool from Ask: multi-step, tool-composing investigation of an open-ended question, planning a NEW combination of already-reviewed analysis functions per question rather than one fixed lookup; refusal is a closed-enum code decision, never the LLM's own self-report; a mandatory Caveats section on every report; the two tabs cross-link on refusal, each using its own reliable structured signal to suggest the other; toolbox covers both the sell-side and, since 2026-09-20, the buy-side self-vs-engine comparison — "is my own buy instinct beating the app's picks?" — each new toolbox function requires a re-confirmed 34/34 refusal-eval pass before it's trusted). **Every surface here is advisory — it narrates and reflects, it never originates or gates.**
...
| `_run_broker` | Daily SnapTrade broker sync (F-244, added 2026-08-18 — missing from this table until 2026-09-02): pulls Robinhood positions/balances/transactions, writes `broker_position_snapshot` + `account_cash` + `account_flows`, and stages content-matched trades into `snaptrade_pending_imports` for human review on 📒 Trade Journal. Fires 2x/day; never auto-writes to `trades` |
