...
| **C1** | ✅ **DONE 2026-09-24.** Benchmark Mirror's headline result (beta-adj. alpha, falling back to raw alpha) now shows on 🧾 Summary's Horizon zone, with a → My Edge pointer that renders unconditionally. **Real constraint found and disclosed, not hidden:** the underlying fetch is an unmemoized `yfinance.download()` gated behind My Edge's own manual "Load benchmark data" button (cost control, not a bug) — so per `feedback_cross_page_mirror_check_reachability`, Summary reads My Edge's published `_bm_summary_cache` (new coordination cache, documented in CLAUDE.md) rather than recomputing, and shows the number only when it was computed THIS session, today; otherwise just the pointer. A cross-session-persisted version would need a DB column + its own review — deliberately out of scope here. | `app.py` (Summary zone, My Edge publish site), `stock_analyzer/util.py::benchmark_mirror_summary_state` | M | No |
