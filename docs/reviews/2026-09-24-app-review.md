...
| **G1** | ✅ **DONE 2026-09-24.** User Guide's 🎯 My Edge bullet now says "six" tabs and lists 🧭 Self vs Engine (confirmed against the real `st.tabs()` call — 6 tabs, not 5). | `app.py:37604` | XS | No |
| **G2** | ✅ **DONE 2026-09-24.** All 3 in-guide sites now say "Business Quality" (glossary Composite bullet, the "More Buy Candidates" explanation, and the AI-integration diagram label) — confirmed the live pillar name in `scoring.py`/`constants.py` is `business_quality`/"Business Quality", never "Fundamental". | `app.py` (3 sites) | XS | No |
