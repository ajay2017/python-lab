...
| **K1** *(new, found during J2)* | The premarket lane's velocity check (`_run_premarket`, accelerating-WATCH-ticker detection) also calls the unsafe `db.load_exit_signals()`. Same bug class as A1/J1/J2, but lower stakes — awareness-only, and fails safe (an outage collapses to `velocity_alerts=[]`, i.e. no false alert, never a false one). Not fixed here — deliberately left as its own item rather than silently expanding this commit's scope. | `cron_runner.py:400` | S | Unverified — confirm before assuming |
