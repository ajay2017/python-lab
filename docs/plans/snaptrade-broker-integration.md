...
**All 6 build chunks complete and Opus-reviewed.** Docs synced same session: `docs/requirements.md` (new F-244), `docs/architecture.md` (+2 module sections for `snaptrade_client.py`/`broker_sync.py`, +3 new §6.33–6.35 table sections, `trades.broker_txn_id` documented in §6.3, constants table from chunk 1), in-app User Guide (💰 Account tab + pages-at-a-glance). **Remaining before this is live:** (1) commit + push (not yet done — this plan doc was updated ahead of the commit since the doc-sync Definition of Done runs same-session as the code); (2) the user must apply the DDL for `snaptrade_config`/`snaptrade_pending_imports`/`snaptrade_income_events` + the `trades.broker_txn_id` column/index manually via the Supabase dashboard (same convention as every other optional table in this app — see `stock_analyzer/db.py`'s module docstring for the exact SQL); (3) the user must add `SNAPTRADE_CLIENT_ID`/`SNAPTRADE_CONSUMER_KEY` to Railway → Variables and redeploy before the Account page's "Register with SnapTrade" button will do anything; (4) `docs/shipped-log.md` gets its entry at actual commit/ship time (that log is for post-ship history, not pre-commit work), and this plan doc's own Status line should be bumped to SHIPPED then too.

Supersedes the Robinhood MCP path (rejected — interactive-auth only, no headless cron possible; see memory `project_today_pnl_scope`). SnapTrade is REST middleware: one-time OAuth setup produces a `USER_SECRET`, then all calls (app + cron) are headless.

---

## Why SnapTrade over MCP

Robinhood has no public REST API for individuals. The MCP path (`agent.robinhood.com/mcp/trading`) requires interactive/desktop auth with no long-lived headless token — neither the Railway app nor its cron lanes could be the MCP client. SnapTrade solves this: CLIENT_ID + CONSUMER_KEY (app-level, Railway secrets) authenticate the app; USER_ID + USER_SECRET (user-level, generated once during OAuth) authenticate calls against the connected Robinhood account. Both pairs are plain REST/SDK calls — no interactive session required after setup.

## Credential storage

- `SNAPTRADE_CLIENT_ID`, `SNAPTRADE_CONSUMER_KEY` — Railway env vars (app-level, same tier as `SUPABASE_KEY`).
- `SNAPTRADE_USER_ID`, `SNAPTRADE_USER_SECRET` — **also Railway env vars** (user's explicit choice, not a DB row). SnapTrade issues `USER_SECRET` exactly once at registration time; the app cannot silently persist it for the user, so the one-time connect flow displays it and the user manually copies it into Railway → Variables → redeploy. The setup UI therefore shows a 4-step instruction flow, not a single "Connect" button. `snaptrade_config` (DB) stores only non-secret state: `brokerage_authorization_id, status, connected_at, last_full_sync_at` — never the secret itself.
