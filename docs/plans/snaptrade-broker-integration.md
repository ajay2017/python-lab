...
**Remaining before this is live:** (1) commit + push these credential-model fixes (constants.py/`db.py`/`cron_runner.py`/`system_health.py`'s DDL, cron lane, and gate registrations from the main build are already committed and pushed — see git log; only `snaptrade_client.py` + `app.py`'s setup-flow rewrite from this correction are pending); (2) DDL already applied by the user; (3) `SNAPTRADE_CLIENT_ID`/`SNAPTRADE_CONSUMER_KEY` (Personal key values) already added to Railway by the user — no second credential pair needed now; (4) `docs/shipped-log.md` gets its entry at actual commit/ship time (that log is for post-ship history, not pre-commit work), and this plan doc's own Status line should be bumped to SHIPPED then too.
