...
**CORRECTION 2026-08-30:** Phase 2's `judgment_grades` table (the grading harness) was found, via a data-integrity audit, to never have actually existed in production — the DDL was never applied despite Phase 2 being recorded as shipped on 2026-08-03. The grading UI and all wiring code were correct; the database table simply had no backend. The user applied the missing DDL 2026-08-30. The opinion-capture half (`judgment_opinions`, Phase 0/1) was separately confirmed to have been persisting correctly the entire time. **Both tables are now live in production.**
