...
**Still unbuilt — two GATE-tier caches with no freshness banner at all.** The
widened registry now knows about staleness it cannot report, because neither
consuming page calls `_render_portfolio_stale_banner`: **🧑‍⚖️ The Judge** reads
`_reduce_calls` (gate tier, feeds `audit_coherence`) and **📋 Watchlist** reads
`_grow_today_sectors_cache` (gate tier — its own copy calls it a sector-overlap
gate on Ready-to-Enter) plus `_risk_high_alerts_cache`. Judge first: it is a
single read with no competing banner. Watchlist needs a placement judgement
rather than a tacked-on call — it is the ENTER NOW / sizing surface and already
carries its own `_wl_brief_offline` gate-disclosure banner, so a second one
beside it needs deliberate ordering. **Neither is a regression** (both pages
behave exactly as they did before this change); what is new is that the gap is
now visible. Recorded here rather than left in a review transcript, per DoD
step 6.
