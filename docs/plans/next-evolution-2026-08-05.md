...
**Status:** BRAINSTORM — pre-decision. Nothing here is approved, planned, or authorized to build. This is a captured "what's next?" thinking session, not a spec.
...
- **Lessons Learned → recurrence.** ⚠️ **Open question: does a saved lesson resurface when a structurally similar setup recurs?** If not, the F-195 Pattern Library is write-only — a graveyard. A "you wrote this lesson on a setup like the one in front of you" callback would be a cheap, high-value loop-closer.
