...
**Status (updated 2026-09-07):** Phase 2 (earnings-move magnitude, §Phase 2 design below) is **BUILT and Opus-reviewed, SHIP — ready to commit.** Before build, a fresh `planner` pass resolved three implementation-level gaps the original 2026-09-04 design left open (see "§Phase 2 gap resolution" below): unknown BMO/AMC timing (exclude, never guess), exactly-one-row-per-event idempotency across a missed cron day (window trigger + existence check, no upsert-key change), and reschedule invalidation within the existing schema (a new `db.withdraw_unmatured_model_prediction()`, race-safe by construction against the maturation cron). `implementer` then built to that resolved spec: `stock_analyzer/earnings_move_forecast.py` (new pure module), `cron_runner.py::_write_live_earnings_predictions`/`_mature_earnings_predictions` (new EOD-lane steps 7/8, independent of the Phase 1 vol steps, which are untouched), `db.withdraw_unmatured_model_prediction()`, the second Model Lab section (`app.py`), 4 new constants, and tests. **First Opus reviewer pass: FIX-FIRST, 1 blocking** — `_mature_earnings_predictions` ran its reschedule backstop BEFORE the maturity gate, so a still-upcoming pending row's fresh `resolve_upcoming_earnings` lookup legitimately returned its OWN frozen date (gap==0), which the old `0 <=` bound in `is_reschedule` misread as a reschedule — withdrawing every pending row the same run it was written, which would have kept the ledger permanently empty despite the cron logs showing plausible "1 written / 1 withdrawn" churn every day. Fixed by reordering the maturity gate first (so a fresh lookup can no longer return the frozen date once the print has passed) and tightening `is_reschedule` to `0 <` as defense-in-depth, with a new regression test asserting a still-upcoming row is left completely untouched. **Second reviewer pass: SHIP, 0 blocking.** Full suite 5098 passed (+50 over the pre-build 5048); antipattern + constants-doc gates green. No backfill (live-only, by design) — first matured rows will take one earnings cycle (~1 quarter) per held ticker at minimum, and `EARNINGS_MOVE_BASELINE_K`(6) matured prior prints per ticker before the survivorship floor even lets a row be written, so real skill numbers are a multi-quarter horizon away.

**Status (updated 2026-09-04, historical):** Phase 1 SHIPPED and LIVE since 2026-08-06 as **F-234**, accruing real predictions daily via the EOD cron. **Checkpoint reached 2026-09-04:** a real bug was found and fixed first (commit `f0eac96` — `db.load_model_predictions()` was silently truncated by PostgREST's 1000-row cap, undercounting live-matured rows; a follow-up commit `5ef6f72` fixed the resulting MAE-precision display bug and added plain-English captions to the Model Lab page). Once trustworthy, the page showed the **first live-only skill reading: +1.6%, n=28** — crosses the `PREDICTION_MIN_MATURED_N`≈20 floor for the first time, meeting Phase 2's trigger ("Phase 1 has real matured rows"). **Phase 2 is now DESIGNED (below, §Phase 2 design — earnings-move magnitude) but deliberately NOT STARTED — the user chose to document it and pick it up later, no trigger date set.** Phase 3 scope is unchanged and untouched.
