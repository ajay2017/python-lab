...
**Status:** Tier 2 (company-name ticker resolution) SHIPPED 2026-09-07 — `_resolve_company_alias()` reuses `broker_screenshot.py`'s existing `_TICKER_MAP` as a deterministic fallback for when the parse model leaves `ticker` null on a company-name question (e.g. "what's my position in Apple"). Fires ONLY on a null model ticker — never overrides one the model did extract — word-bounded matching, excludes `_LOW_CONF_NAMES` (private-company placeholders with no real tradeable ticker, e.g. SpaceX). One shared alias table, not a second independently-drifting one. Never gates; pure-additive to the parse step, no `_GATE_FILES` touch, no reviewer required. 11 new tests; full suite 5033 passed. Designed + built by lead (Sonnet 5) directly — the scope was a small deterministic fallback reusing an already-vetted table, no threshold/gate/scoring decision involved. **Same-day live screenshot caught a real formatting defect** (not a narrator hallucination): six dollar-figure call sites in `facts_to_text()` (holding_lookup's avg cost/current price/market value, portfolio_summary's total value, sector_composition's per-sector value, rec_outcome's price-at-surface/horizon) were interpolated bare instead of `:.2f`, so a whole-number value rendered as "$210.0" instead of "$210.00" — the narrator correctly echoed the figure exactly as given; the bug was upstream. Fixed all six; `price_at_surface` also wrapped in the existing `_f()` helper since it's the one raw, never-cast value among them. 5 new regression tests. **Tier 2 continued (2026-09-07), analyst-coverage cross-reference:** `rec_outcome` now surfaces the nearest saved `analyst_coverage` row (new `_nearest_analyst_coverage()`, closest article within a new `QA_ANALYST_COVERAGE_WINDOW_DAYS`=30-day window either side of the rec date — deliberately separate from `ANALYST_COVERAGE_FRESH_DAYS`/`WORKFLOW_ANALYST_LOOKBACK_DAYS` so retuning either can't silently move this one). Optional `coverage_df` param, same tri-state pattern as `port_df`/`trades_df`; `app.py` loads it via `db.load_analyst_coverage_or_none()` specifically (not the plain loader) so a DB failure reads as "not checked" rather than "zero coverage." 10 new tests. Tier 1 usability pass SHIPPED 2026-09-05 — three new snapshot intents (`holding_lookup`, `portfolio_summary`, `sector_composition`), read-only over the already-computed `port_df`, no gate/threshold/DB change. `portfolio_summary` was deliberately scoped to a snapshot of right now, not a time-boxed return (a specific-past-period question routes to `unsupported` rather than reinventing the app's existing Modified-Dietz period-return methodology under the same name). **Same-day live screenshot review caught two real defects, both fixed:** a tz-mismatch bug that had silently broken `rec_outcome`'s forward price-move calculation for every query since 2026-08-01 (not just old recs — `fetch_price_history`'s tz-aware index vs a tz-naive comparison Timestamp raised inside a broad except-block), and a narrator hallucination (summed several given dollar figures into a new total, then mislabeled it 1000x too large — "$14.9M" instead of ~$14,860) that also surfaced a second gap (the narrator confidently called a 2.5-month-old recommendation "very recent" despite never being told today's date). `_NARRATE_SYSTEM_PROMPT` gained three guardrails as a result: never combine facts into a new total or rescale a dollar figure, never characterize date recency, use the facts' own terminology (sector ≠ position). **Fixing the tz bug immediately surfaced a third, dependent crash** (masked by the tz bug since 2026-08-01): `price_at_horizon` computing while `price_at_surface`/`pct_move` stayed `None` (no starting price on record) crashed `facts_to_text()`'s `:+.1f` format spec on `None` — fixed by formatting each independently instead of assuming they travel together. **A fourth gap, same day:** `rec_outcome` never mentioned a ticker currently held — the only holding-adjacent check was `acted_on` (a BUY within the Pre-Mortem match window of THIS recommendation), a different question from "is this held right now." Added an optional `port_df` param + `currently_held`/`current_shares` facts, narrated as a standalone line independent of `acted_on`. Full detail in `docs/requirements.md` F-225. Original v1/v1.1/v2/v2.1 SHIPPED 2026-08-01/2026-08-02 below.
