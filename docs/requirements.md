...
| F-39g | **Gate-funnel Sankey (decision transparency).** Grow Today shows an optional Plotly Sankey funnelling today's candidates → outcome: **New Positions to Initiate** vs. the recorded drops (composite below the Buy bar, sector at the hard cap, imminent macro event, couldn't-verify). Counts come from `daily_briefing.gate_funnel_counts()` reading the existing new-position buckets — **no fabricated universe**: the source node is *candidates that reached a recorded gate decision*, and the caption states that names filtered earlier (already in Act Today, hard cross-ref conflict, flat-day caution, selection overflow) are silent and not shown. Add-to-winner has its own gates and is excluded. Display-only; shown only when ≥ 2 candidates were evaluated and at least one was dropped. |
