...
| F-161 | **Missed Opportunity analysis.** Collapses recommendations the user **never acted on** to **distinct tickers** (`distinct_missed` — a ticker acted on in any surfacing is excluded; graded from its FIRST surfacing = the full opportunity window), split into **missed winners** (rose) vs **dodged losers** (fell) with named outliers (biggest miss / biggest dodge, `missed_split`) and a ranked green/red horizontal bar of skipped-name outcomes. Magnitude is framed **per-$1k notional** (a read on *selection* quality) — **never** a fabricated portfolio-level "you would have gained X%" counterfactual, because capital, the 15%/35% concentration caps, and the entry gates all bind (buying every surfaced name is impossible). |
