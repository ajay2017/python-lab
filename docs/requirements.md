...
| G-15 | Fundamentals absent from all sources (< `FUNDAMENTALS_GATE_MIN_METRICS`) → Analysis verdict AND Brief new-pick | Withhold the verdict: on Analysis, BOTH the Summary Scorecard row (Score → "—", Signal → "🚫 Withheld") and the Detailed Analysis block (red "verdict withheld" note) suppress the fabricated-neutral-50 verdict — the quick-glance and detail surfaces stay consistent. Price-derived columns (Stop, Target, R:R) remain (that data is real). `daily_briefing` holds the ticker out of `new_picks` (→ `composite_unavailable`). |
