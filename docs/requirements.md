...
| F-154 | **Analyst Coverage — Ideas Inbox (F-6, Phase 1).** Captures professional analyst research (CNBC Pro, broker notes) into structured, queryable intelligence that both **surfaces new tickers** as watchlist candidates and **enriches** names already held/watchlisted. **Input = paste raw article text** (the source is paywalled/bot-protected so the app cannot fetch it; the user's subscription lives in their browser). The LLM (`analyst_intel.extract_report`, Sonnet) extracts **only atomic per-firm facts** — firm, analyst, rating, price_target, upside_pct — plus ticker/company/date/report_type/thesis/catalysts/risks, and is instructed to pick the **primary** subject (ignoring sidebar/consensus widgets for other tickers) and never fabricate a number (`null` for anything not stated). **All aggregates (avg/high/low PT, consensus label) are computed in pure Python** (`analyst_intel.derive_consensus`), never by the LLM, so no arithmetic is hallucinated. The extraction is shown in an **editable preview** (the human safety net — the user corrects any misread before save) and the **raw text is persisted** (`analyst_coverage.raw_text`) for re-processing. Saved rows render as an **Ideas Inbox** card library (recent = within `ANALYST_COVERAGE_FRESH_DAYS`, 30) with held/watchlist badges and per-card **▶ Analyze** (routes the ticker to the 📈 Analysis page → existing 5-gate scoring) + **➕ Add to Watchlist**. Awareness-only: analyst PTs/ratings **never modify a composite score, gate, or verdict** — the value is the **"Wall Street vs. your engine" tension** (e.g. five firms rate a one-month-old IPO Buy while the engine scores it below `COMPOSITE_BUY` (65) on thin history). Strictly additive / zero runtime dependency: extraction returns None offline; ships **inert** until the additive `analyst_coverage` DDL + service-role RLS are applied (load returns empty). Consensus **label** boundaries (`ANALYST_CONSENSUS_STRONG_BUY_FRAC` 0.80 / `ANALYST_CONSENSUS_BUY_FRAC` 0.50 / `ANALYST_CONSENSUS_SELL_FRAC` 0.50) are **display-only classifications, not decision thresholds**. `ANALYST_MIN_UPSIDE_PCT` (15) reserved for the Phase-2 Brief chip (unused in Phase 1). Plan: [docs/plans/analyst-coverage.md](plans/analyst-coverage.md). |
