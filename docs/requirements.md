...
| G-19 | **Concentration-gate basis — "tighter-of-both" (margin-aware).** The single-name (15%) and sector (35%) hard gates (G-16, F-39b) compare a position's **`Gate Weight (%)`** — not the equity-only display `Weight (%)` — against the ceilings. `Gate Weight = MV ÷ gate_denom`, where `gate_denom = min(equity, total_account_value)` and `total_account_value = equity + signed net cash` (negative cash = margin debit, per F-03b). Effect: **margin tightens** the gate (smaller net-capital denominator → weights run higher → ceilings bite sooner); **cash on hand never loosens** it (the equity denominator is kept). If the manually-entered cash figure is **unknown or stale** (> `ACCOUNT_CASH_STALE_DAYS`, 7) the gate **degrades to equity-basis**; if net capital is **≤ 0** (over-levered) all adds are suppressed. `gating_denominator` in `concentration.py` owns the rule; the `Gate Weight (%)` column is injected once at the data boundary. **Display weights everywhere else stay equity-basis.** A visible "⚖️ Tightened by margin" annotation renders on the suppression banners / entry nudge / trim recs whenever the account-basis is active (never silent). Investment-policy decision 2026-06-26. **All concentration surfaces now share the basis: hard gates + entry nudge (Phase 1), and the `risk_advisor` `single_name_concentration` / `sector_concentration` trim recs + the watchlist / quick-research / comparison entry-fit checks (Phase 2).** In the trim recs the displayed weight, the gate, AND the trim pp/$ are all account-basis together (every weight scales by `equity/gate_denom`; the "$ at risk on a name" is unchanged market value, only the trim amount grows). Display weights elsewhere (incl. beta/Sharpe recs — a different risk dimension) stay equity-basis. |
...
| F-06a | **Composition Sankey (Portfolio → Sector → Position).** On the Overview tab, a Plotly Sankey shows where capital sits and how concentrated it is in one view; band width = share of portfolio value. **Concentration reads at a glance:** a sector node over `SECTOR_CEILING` (35%) or a single-name node over `SINGLE_NAME_CEILING` (15%) is flagged red. Hover shows each node's % of portfolio. Display-only (the gates themselves fire elsewhere). |
| F-07 | Show sector exposure breakdown as a percentage of portfolio. The concentration reference line is drawn at `SECTOR_CEILING` (35%, the actual hard cap — never a hardcoded literal). |
...
| F-12b | **Concentration-gate basis is margin-aware ("tighter-of-both").** The hard single-name/sector gates and the entry nudge measure concentration against `min(equity, total_account_value)` rather than invested equity alone, so trading on margin tightens the ceilings while cash on hand never loosens them. Display weights stay equity-basis; only the gate comparisons use the net-capital `Gate Weight (%)`. Falls back to equity-basis when the cash figure is unknown or stale (> `ACCOUNT_CASH_STALE_DAYS`). Implements G-19. (Phase 1: hard gates + entry nudge. Phase 2: `risk_advisor` trim recs — weight, gate, AND trim-$ re-based together — plus the watchlist / quick-research / comparison entry-fit checks.) |
...
| F-75 | **Opportunity Signals ↔ Brief reduce-call reconciliation.** The Overview's "Opportunity Signals" (positive news on held names, composite Score ≥ 55, sentiment ≥ +0.1) never frames a position as "may support adding on a pullback" while the Daily Brief carries an active **Reduce/Exit call** on the same ticker — the protect-capital directive leads the composite (the deterioration ladder fires before the composite reacts), so an add prompt there would contradict the Brief (the PLTR deterioration-exit case). It consumes the Brief via `decision_bucket.reduce_call_tickers()` — the SAME `_is_reduce`/`_ticker` canon the Act-bucket reconciler uses (act-origin stop / sell / risk / deterioration / risk-off reduces AND review-origin trim variants, across both the Act and Awareness buckets, since deterioration cards bucket to Awareness) — so the two surfaces can't drift. Suppressed names are NOT silently dropped: an amber "⚠️ NOT SHOWN AS ADDS" note lists them with the reason, and the full headline still appears under "All News for Your Holdings." Display-only — no gate/score/verdict change. |
