...
| F-143 | **Once-per-ET-day idempotency + DST safety.** GitHub cron is UTC with no DST, so each job is scheduled on TWO UTC slots straddling EST/EDT; `cron_runner` guards on trading-day + ET-hour + a per-day Supabase `alert_state` flag (row 1 = premarket/protective lane, row 2 = EOD pullback lane) so exactly one run fires per ET day per lane — never a double-send. The mode is derived from ET hour (< 12 ⇒ premarket, else eod); only `ALERT_RUN_MODE=scan` is respected as an env-var override — `premarket` and `eod` override values are intentionally ignored so YAML schedule changes cannot accidentally fire the wrong lane. The `workflow_dispatch` manual trigger exposes `mode` choices `auto` and `scan` only (auto = hour-derived). |
