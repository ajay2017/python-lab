...
└── stock_analyzer/                 Domain logic package
    ...
    ├── broker_sync.py              SnapTrade broker-sync transform/decision logic (pure) — diff_positions, map_balances, classify_transactions, annotate_pending
    ...
    ├── db.py                       Supabase ops (holdings/watchlist/trades/manual_stops); trade-replay; fractional shares; reference-table persistence
