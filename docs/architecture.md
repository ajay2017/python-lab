...
| One risk → one surface (dimension-scoped dedup) | Each risk surfaces ONCE on its highest-priority surface. `_review_list` receives `act_today` and skips its negative-news WATCH for any ticker already flagged in Act Today (like `_buy_candidates`/`_grow_today` already do). Dedup is by risk DIMENSION, not by ticker: AVGO may still show two Review cards (earnings WATCH + stop-raise ACT) because those are genuinely different actions. | DELL appeared as a Critical-News ACT in Act Today AND a "no action today" WATCH in Review — same news, contradictory directives. The Brief is a quick-glance action list, not a scavenger hunt; but blindly deduping by ticker would hide genuinely distinct actions, so the line is "same risk, or different one?" (Commit 02ed2da.) |
