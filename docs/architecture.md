...
| Diversification ADD candidates cross-validated | The Diversification Advisor proposes candidate names from a static per-sector roster (it answers "which sector is underweight," not "is this name a good entry"). The Risk tab now joins each candidate to the SAME composite/signal/R:R the Analysis page produces — via `portfolio.annotate_add_candidates` fed from the cached `_grow_composites` bundles (load_all fallback for un-scored names) — gates against `COMPOSITE_BUY` (65), sorts best-first, and shows a 🎯 banner naming the best gate-passer (with R:R) or a 🚦 "sector tilt sound but no name clears the gate" banner. Failing names stay visible-but-demoted with a "Below Buy gate" note (never silently filtered). | Visa surfaced as a rebalance ADD-to-Financials candidate but never as a new-position pick; the rebalance card hid that V actually scores Buy 65.7 with a 5.9:1 R:R. The sector-tilt engine and the quality engine were two disconnected reads — now they give one. |
