...
| Broker screenshot import (Claude Vision) | `stock_analyzer/broker_screenshot.py` (pure — no Streamlit or DB imports) parses one or more PNG/JPG screenshots of the Robinhood Account → History page via Claude Vision (Opus 4.8). The Anthropic `anthropic` client is imported lazily so the module loads without the package on cold environments. Multi-image dedup operates on a content key `(ticker, action, date_str, shares, price)` so overlapping screenshots don't double-import. `_infer_year()` resolves partial dates like "Jul 9" to full calendar dates using the reference date (upload date), with prior-year fallback for future-looking dates. `find_app_only_in_range()` builds a content-match multiset against screenshot trades to surface "in app, not in screenshot" discrepancies (not date-exact — app `traded_at` may differ by a day from Robinhood activity dates). `last_screenshot_sync_date()` reads `trades.notes` for the "RH screenshot YYYY-MM-DD" tag to default the sync-since date picker. Write path is identical to the CSV import path (human-in-the-loop `st.data_editor` preview → `db.save_trade` + `recalculate_from_trades`). **No auto-deletes ever.** (Commit 97590fb; reqs F-87b.) | The Robinhood CSV export has field-parsing quirks handled by `broker_import.py`. Screenshots are the other natural "what happened" record. A Vision-based path adds a second reconciliation channel without requiring the user to navigate to the CSV export flow; it is particularly useful when only the History view is visible on mobile. Low-confidence tickers (private companies, ambiguous names) are flagged rather than silently accepted so the user verifies before any data lands in the trades table. |
