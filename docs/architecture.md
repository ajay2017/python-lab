...
| Catalyst Watch (awareness, not a gate) | Its OWN nav page (🔔 Catalyst Watch, before Economic Calendar — moved off the Home brief to keep it lean; viewable any time). Lists upcoming earnings (≤ `CATALYST_WATCH_WINDOW_DAYS`) for tracked names (held ∪ watchlist ∪ sector universe), grouped Today/Tomorrow/Next-7d, tagged sector + 🔥 leading flag + held/watchlist/universe. `catalyst_watch.build_catalyst_watch` (pure) intersects an earnings calendar with the tracked set; the calendar comes from FMP market-wide (`data.fetch_earnings_calendar`, one call) THEN a per-name yfinance fallback (`data.fetch_next_earnings`, threaded) for names FMP didn't cover — so universe coverage doesn't depend on FMP's tier. 24h-cached; dedicated page means the heavier fetch runs only on visit. | The app intentionally won't RECOMMEND buying into earnings (binary risk), but it was BLIND to a tracked name reporting (PANW beat after close, nothing surfaced). FMP's free calendar returned held-only, so the yfinance fallback was added. Awareness ≠ recommendation. (Commits dd8aea5 / eead6b0.) |
