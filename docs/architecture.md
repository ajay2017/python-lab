...
└── stock_analyzer/                 Domain logic package
    ...
    ├── util.py                     Safe-idiom helpers closing audit-found bug classes (offline sentinel preservation, HTML escaping, tz)
    ├── market_time.py              Single source of truth for NY-timezone now()/today() and market calendars
    ...
    ├── valuation.py                Valuation pillar scoring — Forward P/E (sector-relative), FCF Yield, analyst PT upside/consensus; split out of fundamentals.py to give valuation its own explicit weight in the 4-pillar composite; graceful degradation (absent metrics contribute 0 to both numerator and denominator)
    ...
    ├── capital_vs_margin.py        Capital vs. Margin Analysis (F-267): backward account-value reconstruction from broker go-live + margin cost/benefit verdicts — interest sign-isolation · cash reconstruction · holdings reconstructed forward from trades (fixed 2026-09-11: gross book previously lagged cash by days, producing a real artificial drawdown) · price-lookup fallback chain · gross-book/daily-returns · self-validation (cash hard-gates, gross drift advisory-only) · dual equity curves · margin contribution · break-even rate · drawdown decomposition · projected interest · deleverage scenario · regime split · `window_income_events()` (fixed 2026-09-11: the interest headline previously had no date filter and summed pre-go-live events, mismatching the exposure-P&L window it's netted against — see F-267's 4th bug); its income-event inputs are also deduped via `broker_sync.dedupe_income_events()` (F-268 cross-path fix) before reaching this module. 26 pure functions (55 tests), zero I/O, zero policy thresholds
    ...
    ├── summary_view.py             🧾 Summary cockpit view-helpers (pure, no Streamlit/DB): book_safety (5-state drift + never-green-without-evidence),
    │                               quote_change_pct (single quote reader), movers_window (today vs last session), top_movers, score_tone (reuses the
    │                               rec bands, invents no cutoffs), weight_bar_pct, position_status_badge, day_direction_counts, avg_score_label
    ...
    ├── beta_repair.py              Beta-lever arithmetic (F-274): trim/swap/add dollar-to-target formulas + relief comparison, window-aligned candidate beta (aligned_beta), margin/leverage side-effect (leverage_side_effect — 4-state, never fabricates a direction on unmeasured data), and a rank-never-recommend defensive-candidate classifier (rank_defensive_candidates). Zero I/O, in `_GATE_FILES`. Called from risk_advisor.py's beta rec (trim math) and app.py's Risk Analysis render (leverage disclosure, candidate ranker) — never from home_risk_synthesis.py, since the leverage/reduce-call cross-checks need account-cash access and a downstream cache the producer can't see without circularity
    ...
    ├── exit_velocity.py            Exit-signal velocity: detects a held position's WATCH-tier signal accelerating toward TRIM so an alert can fire BEFORE the TRIM threshold is crossed; pure, consumes the already-loaded `exit_signals` DataFrame; silently returns None for <2 days of WATCH history
    ...
    ├── decision_context.py         Decision-context capture (Concept E) — freezes state at interactive BUY/SELL write as schema-versioned JSON blob
    ├── decision_quality.py         Decision Quality retrospective analytics (monthly/quarterly grades, prep-tier classification)
    ├── behavioral_fingerprint.py   Behavioral Fingerprint — Concept A (F-193), My Edge 4th tab: observation-only, sample-gated statistics over the SAME recommendations/trades substrate as recommendations_history.py/decision_quality.py (never a parallel logger); buy-side only for v1; every pattern function returns None below its min_n floor rather than present a directional finding at small N
    ├── predictive_analytics.py     Predictive Analytics — signal calibration helpers (Option A); pure functions over recommendations_history.compute_outcomes() output; lazily called from the "📊 Predictive Analytics" page (F-178)
    ...
    ├── broker_screenshot.py        Robinhood History text parser (pure — parses pasted trade text into structured records)
    ...
    ├── self_track_record.py        Self Track Record (🎯 Summary) — classifies BUYs as app_aligned / self_in_scope / self_out_of_scope / coverage_limited for user-vs-engine grade
    ├── protective_track_record.py  Defense facet of 🎯 Engine Track Record (F-229 Phase 2) — grades EXIT/TRIM protective signals on sign-flipped alpha (underperform = right)
    ├── investor_mirror.py          Conviction Alignment + Behavioral Bias analytics (F-194) — Spearman rank correlation, orphan/overexposure/overhang patterns
    ├── personalized_discovery.py   Personalized Discovery badge — flags Grow Today picks resembling realized winning trades (winner-profile match)
    ├── trade_time.py              Re-anchor imported trades to ET time (fixes midnight-UTC mis-dating on import sources)
    ...
    ├── earnings_intel.py           Earnings Intelligence Phases 1 & 2 (F-174): extracts structured earnings data from raw CNBC Pro/broker article text via LLM, same idiom as analyst_intel.py — strictly additive (every public function returns None on any failure), zero-hallucination (LLM extracts only atomic stated facts, aggregates/posture computed in Python)
    ├── analyst_targets.py          Analyst consensus PT cut detection (F-169 Phase 2) — firms keeping rating unchanged but cutting PT are structurally invisible to action-only alerts
    ├── thesis_red_team.py          Thesis Red Team Agent (F-198+F-199, Concept I) — quantitative erosion score + Haiku counter-evidence narrative + Pre-Mortem loop
    ├── portfolio_thesis.py         State of the Portfolio standing thesis (F-232) — weekly stability ledger (no LLM, pure composition of existing computed values)
    ├── thesis_cluster.py           Hidden Same-Bet Detector (F-199, D1) — clusters held tickers' saved theses by common underlying bets, pure + LLM-fabrication guarded
    ├── judgment_opinion.py         Judgment-layer opinion schema (Phase 0) — common shape for all witness subsystems + PROTECTIVE_DIMENSIONS set
    ├── judgment_grading.py         Judgment-layer grading harness (Phase 2 "The Judge") — grades individual witnesses vs realized outcomes
    ├── judgment_synthesis.py       Judgment-layer synthesis (Phase 1) — reconciles opinion list into per-ticker + portfolio posture, evidence-weighted (Phase 3)
    ├── ticker_history.py           Prior Trades F-237: per-ticker round-trip reconstruction from the trade journal — build_ticker_history (0→held→0 episodes, weighted-avg cost mirroring db.recalculate_from_trades incl. SPLIT-overwrites-state — with one deliberate divergence: a SPLIT with NO open episode is skipped rather than seeding state, since there's no round trip to attribute it to, so a later SELL becomes an orphan_sell here where db computes a basis. A SPLIT INSIDE an open episode rescales that episode's legs onto the post-split basis (shares×f, price÷f) or realized_pct is wrong by the split factor; each SELL's dollar P&L is frozen at append time so a recomputed leg isn't rescaled with the per-share figures. Realized P&L summed from the STORED SELL realized_pnl so it can't disagree with Portfolio; orphan-SELL / split-in-window / oversell warnings; None on offline vs [] on never-traded — but note db.load_trades returns an empty DataFrame on read failure, so the None sentinel is unreachable through that producer and the consumer must treat an empty journal as indeterminate) + build_pnl_series (per-episode P&L% vs running basis + "had you held" ghost after the last exit); `_build_journal` also reads `situational_category` (F-257, BUY-only) off the opening row for F-237e's tab badge; pure, awareness-only, never gates
    ├── ticker_liveness.py          Weekly ticker-liveness sweep for hand-maintained rosters — answers which curated roster tickers have stopped trading (F-241, maintenance cron)
    ├── bundle_loader.py            Shared market-data bundle loader (load_all) — the app AND the headless cron load through the SAME path
    ├── snaptrade_client.py         Thin wrapper over SnapTrade SDK (personal-key mode, F-244); returns None on any failure (offline sentinel)
    ├── broker_sync.py              SnapTrade broker-sync transform/decision logic (pure) — diff_positions, map_balances, classify_transactions, annotate_pending, parse_robinhood_csv_income (F-268 CSV importer for SnapTrade/Robinhood gap)
    ├── home_risk_synthesis.py      Home's correlation/diversification + risk-metrics producers (extracted from app.py, F-260 Phase 3)
    ├── coord_freshness.py          Freshness bookkeeping for portfolio-dependent coordination caches (F-260 Phase 1)
    ├── outage_gate.py              DB-outage render gate (extracted decision logic, F-243) — allowlist application, scope branching, message text
    ├── act_today_precedence.py     Held-position banner precedence for add-to-existing flow (stop-breach > reduce call > plain held)
    ├── grow_dropoff.py             Grow Today transparency helpers (firmness badge, drop-off trace; pure logic)
    ├── premortem_advisor.py        Pre-Mortem Protocol (Concept C) — position-specific counterargument + falsifiable pre-commitment
    ├── premortem_monitor.py        Pre-Commitment Enforcement — actively monitors stated pre-mortem triggers, fires Act Today if breached
    ├── intraday_entry.py           Intraday pullback entry window — fires when morning go-verdict pick dips while SPY holds
    ├── gate_registry.py            Frozen APPEND-ONLY gate_id → label map (ids the capture half can emit)
    ├── gate_ledger.py              Gate Suppression Ledger capture-half builder (pure, no DB/Streamlit)
    ├── gate_ledger_readout.py      Gate Suppression Ledger readout (F-259 Phase 2) — forward-alpha measurement vs SPY post-suppression
    ├── regime_targets.py           Regime-Conditional Position Targets (Concept D Wave 3) — diagnostic beta/cash fit vs macro regime
    ...
    ├── db.py                       Supabase ops (holdings/watchlist/trades/manual_stops); trade-replay; fractional shares; reference-table persistence; income-events CSV import (F-268: save_income_events_from_csv); load_snaptrade_income_events now also selects snaptrade_txn_id (CSV vs broker dedup)
    ├── system_health.py            System proprioception — pipeline-trust health checks (Phase 1, F-235). Owner-only diagnostic; every function is read-only/informing, changes no gate/recommendation/composite/threshold. Pull-based/render-time — never depends on its own background job having run; answers "can I trust what the app told me today?" for 🩺 System Trust
...
| Constant | Value | Role |
|---|---|---|
| `COMPOSITE_WEIGHTS_VERSION` | 2 | Provenance stamp for `COMPOSITE_WEIGHTS`' own weight VALUES (not the schema) — bump only when the weights themselves change, never on unrelated edits near the dict. `1` = the retired 3-pillar `{technical .45, fundamental .40, sentiment .15}` (2026-05-05 .. 2026-07-08); `2` = the current 4-pillar split shipped in commit `6dc1197` (2026-07-09 onward) — the ONLY weight-value change in this project's history, confirmed by walking the full git history back to the first scoring commit. Persisted per-row as `recommendations.weights_version` (§6.12), mirroring `SIZING_FORMULA_VERSION`'s own precedent, so a future weight change can never be silently compared against a historical `composite_score` produced under different weights. |
...
| `MARGIN_MAINTENANCE_RATE` | 0.25 | Estimated Reg T maintenance floor used by the 💰 Account margin call-distance panel (F-253). Robinhood raises this on volatile/concentrated names, so the real call threshold is typically closer than this rate implies. **Awareness only — read by NO gate or advisor.** |
...
| `NET_CAPITAL_POSITION_CAP_PCT` | 25.0 | **F-255.** A SEPARATE, capital-basis cap on NEW/added position sizing only — caps a single position at 25% of net capital (gross book minus margin debit), additive to and independent of `SINGLE_NAME_CEILING` (15% of gross book). At real leverage, "15% of book" can be ~45% of capital (ALB/OXY, 2026-08-24), which is the gap this closes. Applied inside the shared `risk.position_sizing()`/`sizing_unavailable_reason()` (default-off when `net_capital` is omitted — every existing gross-basis caller is unchanged) and `concentration.assess_add_concentration()`. Inactive (falls back to the gross cap alone) when the account is unlevered or the seeded cash balance is stale; fails closed (no size suggested) when net capital is ≤ 0 (margin call). Existing holdings already over the cap are surfaced as an awareness-only banner on 💰 Account — never a forced trim/exit. |
...
| `GATE_LEDGER_MIN_CALLS` | 8 | **Display-only** band threshold for the Gate Suppression Ledger readout (F-259, "🛑 The Road Not Taken"). Below this count of matured+priced, `counterfactual=true` suppression rows for a gate, the readout shows "building" and withholds a verdict for that `gate_id`. Pre-registered with the user 2026-08-26 (`docs/plans/gate-suppression-ledger.md` §5, "N_min") before any capture data existed. NOT an investment gate; never feeds `risk_advisor`, `exit_advisor`, `daily_briefing`, scoring, or sizing — this readout is a retrospective measurement of the app's own past restraint. Safe to tune from observation. |
...
| `SELF_TRACK_SELL_RELIABLE_LOG_START` | 2026-07-21 | My Edge → 🧭 Self vs Engine → 📉 Sell-Side — the `exit_signals` cron capture went live this date. Before it, a SELL with no matching EXIT/TRIM signal on file is bucketed `coverage_limited` (disclosed, never graded either way) since the gap could be missing coverage rather than a genuinely self-initiated exit; on/after this date the same shape is graded `self_initiated`. Boundary inclusive (`>=`); a matched signal overrides the boundary regardless of date (date-independent). |
| `SITUATIONAL_CATEGORY_MIN_SAMPLE_N` | 3 | My Edge → 🧭 Self vs Engine → 📈 Buy-Side (F-233 V2, situational category) — minimum matured, graded self-initiated BUYs required **within a single situational category** (`decision_journal.SITUATIONAL_CATEGORIES`) before that category's average alpha-vs-SPY renders; below it the row shows "building". Deliberately NOT `BEHAVIORAL_MIN_SAMPLE_N` (8) — the self-initiated pool splits 5 ways, so an 8-floor would leave every category permanently "building". Measurement-only — never gates, sizes, or suppresses a recommendation. |
...
| `DIVERSIFY_WELL_PCT` / `DIVERSIFY_MODERATE_PCT` | 42.0 / 30.0 | Diversification Score (0-100) label bands: ≥ WELL → "Well Diversified" (avg pairwise correlation ≤ 0.16); WELL > score ≥ MODERATE → "Moderate" (0.16-0.40); below MODERATE → "High Correlation Risk" (> 0.40). Used by `home_risk_synthesis._div_label` and the Home risk-banner RAG classifier (Action Required / Monitor / All Clear). Promoted from a hardcoded 42/30 literal in both sites — same values, not a policy change (2026-09-01 audit Medium finding) |
...
| `STOP_RECOVERY_MARGIN_PCT` | 0.5 | Minimum live `Gap to Stop (%)` (from `_port_df_enriched`, which is rebuilt on live prices each render) for a memoized `stop_breach` Act Today card to be demoted to the "Stop Breach — Resolved" monitoring section. A bare 0.0 would flicker the demotion on/off as price oscillates a penny around the stop boundary; 0.5 is one half-percent of breathing room above the stop before "recovered" is declared. Consumed only at render time in `app.py`'s `_stop_rec_state` call — does not affect the Brief build or the `gap ≤ 0` breach test itself |
...
| `WEEKLY_DEBRIEF_MIN_SNAPSHOT_DAYS` | 5 | Minimum trading days of `daily_snapshots` history required before the Weekly Debrief (F-3/F-152) will generate — below this the week-over-week narrative would be built off too thin a sample. Used by both the Sunday cron lane and the interactive "Generate Now" button (`debrief_advisor.classify_snapshot_read`), centralized 2026-08-30 so the two call sites can't drift apart. Measurement floor, not a decision gate; never affects investment recommendations. |
...
| `RAPID_REVERSAL_WINDOW_HOURS` | 48.0 | 📒 Trade Journal → Log Trade — live SELL mirror, the structural inverse of the `EXIT_SIGNAL_ACT_WINDOW_DAYS` mirror above. Fires a fact-only, non-blocking caption (`classify_rapid_reversal_sell` in `behavioral_fingerprint.py`) when a SELL is logged within this many **wall-clock** hours of the most-recent BUY of the same ticker, with no active exit signal and a still-non-bearish composite tier — surfacing a fast, unprompted reversal against the engine's own still-bullish/neutral read. Wall-clock, not trading, hours: a weekend-straddling hold can exceed this in clock time while short in trading time; accepted rather than adding trading-hours math to a passive caption. Renders outside `st.form(...)`, inside the same outer `try/except: pass` as the sibling mirror — never gates, slows, or blocks the trade write. Owner-set 2026-09-21 after a real-data investigation found two round trips (2 minutes, ~1 day) matching this exact shape. |
...
| `QA_ANALYST_COVERAGE_WINDOW_DAYS` | 30 | Portfolio Q&A — window (calendar days, EITHER side of a recommendation's surface date) to search `analyst_coverage` for the single closest row, for the `rec_outcome` "what was Wall Street saying" cross-reference (Tier 2, 2026-09-07): narrates the nearest saved consensus rating/price target as context, retrospectively only. Deliberately a separate constant from `ANALYST_COVERAGE_FRESH_DAYS`/`WORKFLOW_ANALYST_LOOKBACK_DAYS` (same order of magnitude, different anchor date and consumer) so retuning either of those can't silently move this one. |
...
| `INVESTIGATOR_LLM_TIMEOUT_SECONDS` | 90 | Portfolio Investigator — per-call network timeout for the plan/report LLM calls, passed explicitly by the Investigator's `llm_fn` closure (never touches `ai_provider.call_llm`'s own general-purpose default). Added 2026-09-18, same live-production session as the token bump above: raising `INVESTIGATOR_MAX_TOKENS_REPORT` gave a thinking-heavy model more room to actually use, which made the old 30s default genuinely too short — a real report call hit `anthropic.APITimeoutError` before finishing. |
...
| `_day_shock_cache` | Home page (after `_price_strip`) | Home Day Shock banner · **🧾 Summary Top-Positions ⚡ marker (F-204a, 2026-08-29 — it had been published and read by NOTHING but Home since it was added)** | Held tickers moving ≥`DAY_SHOCK_PCT` same-day, `{ticker: {price, prev_close, chg_pct}}` — AWARENESS, **never gates** |
...
- **Stale-window guard (fixed 2026-08-31):** `YFinanceProvider.live_prices()`'s `prev_close` comes from a 2-day daily-bar batch fetch; right around/after the open, Yahoo's backend can lag publishing the new session's bar, leaving that window one session behind Finnhub's already-rolled-forward `prev_close`. Reported literally, that reads as a real day-over-day price move for the affected tickers and strictly breaches the 0.5% tolerance — 8 unrelated names (APP/CRM/DELL/MSFT/NVDA/SHOP/UBER/WDAY) fired this exact false "sources disagree" alarm simultaneously that morning, confirmed by live-refetching and matching the reported gaps to the tickers' actual Thu-vs-Fri closes. `live_prices()` now nulls `prev_close` (not `price`) whenever today is a trading day (`_is_trading_day`, duplicated from `data.is_trading_day` — importing it would cycle, since `data.py` imports this module) and the batch's last bar isn't from today; the strict leg then has no comparable data and skips for that ticker until Yahoo's window catches up (self-resolves same day, confirmed). On a non-trading day the guard does not fire — the last bar legitimately IS the correct prior session, matching the correct 0.0%-gap read already logged the day before. **Known residual (non-blocking, Opus-reviewed):** the loose live-price leg still reads the same stale bar as `price` during that window, so a volatile open could still breach `DATA_XCHECK_LIVE_TOL_PCT` on its own — pre-existing, not introduced by this fix, and not what fired here (that morning's live-price legs agreed; only the strict prev-close leg breached).
- **Consumers now defer to the cross-check verdict too (fixed 2026-08-31, same day, different provider):** the yfinance fix above didn't fully close the incident — the banner kept firing because Finnhub (the PRIMARY, not the validator) was independently returning a `prev_close`/`change_pct` stale by one session for several tickers, well past the Monday open (confirmed by back-solving the reported gaps against freshly-refetched, already-correct Yahoo data). This doesn't just drive the cross-check banner: Home's Day Shock card and the live price strip's per-ticker %-change badge both read the SAME `_cached_live_prices()`/Finnhub call, so they were reporting Friday's already-happened move as today's ("MRVL -10.27%", "WDAY +5.78%" — both spans were really Thu→Fri). There's no local bar-date to sanity-check on Finnhub's side the way there is for yfinance, so the fix instead reuses the cross-check's own `prev_ok` verdict as the trust signal: the compute (`_xc`, already 5-min cached) is hoisted earlier in Home's render and published as `st.session_state["_xc_bad_prev_tickers"]`; Day Shock excludes a flagged ticker (with a caption naming why), the price strip shows "⚠ unverified" instead of a number, and the Today's P&L (held) Tier-A tile excludes the ticker from its sum with its own caption — **but that caption is explicitly gated on `_dpnl is None`** (Tier A actually being the tile shown), because Tier B (the snapshot-baseline day-P&L) never reads `prev_close` at all and already counts the flagged ticker correctly — an ungated caption would have contradicted a correct Tier-B number (caught by Opus review as a blocking finding, fixed same round). Two Opus review rounds on this leg: SHIP (Day Shock/price-strip) and FIX-FIRST → fixed → SHIP (Today's P&L). **Extended same day to 🧾 Summary** (commit `b2e4cd8`, Opus-reviewed SHIP, 0 blocking): Summary has its own separate read sites for the same data — Today's Movers (`summary_view.top_movers`), its own Tier-A P&L fallback + caption (same `_dpnl is None` gate), the shared `_render_holdings_table`'s Day Δ% column, the Top Positions day-% column, and the day-direction footer (`summary_view.day_direction_counts`) — all now filter a flagged ticker out of the `live_prices` dict passed to these UNMODIFIED pure functions before calling them, so the functions' own existing "absent ticker = missing/no-quote" contract does the exclusion; no `stock_analyzer/summary_view.py` logic was changed. See memory `project_yfinance_prevclose_staleness` for the full incident chain (now closed).
...
| Analyst consensus rating | 15 | Strong Buy=15; Buy=12; Hold=8; Mixed=5; Sell=0 (compressed 2026-09-13, analyst-weight-audit Phase B — see `docs/plans/analyst-weight-audit.md`; was 30/24/15/9/0) |
...
Since the concentration gates are **equity-basis** (2026-07-09 — reqs G-19), `_acct_gate_cache.basis` is always `"equity"`: the **Sector Exposure** chart and **Composition Sankey** show the equity view and flag concentration (35% sector / 15% single-name) against invested equity. Leverage/margin risk is instead an **awareness-only** read on the **🔗 Risk Analysis** tab (`_leverage_cache`; F-09d) + the 💰 Account ⚖️ note — never a gate. **CORRECTED 2026-08-30 (F-260 findings #6/#9):** the net-capital overlay/flag branches this paragraph used to describe as "retained as the seam" were **removed**, not merely inert — a surface-proprioception audit flagged the `_acct_gate_cache` reads feeding them as offline-sentinel collapses, and investigating that found the branches had been unreachable since the 2026-07-09 reversal above (`basis` can never be `"account"`/`"over-levered"` again), so keeping them served no purpose and was actively misleading (a "You carry a margin debit..." caption and a second bar trace that could never render). Re-enabling net-capital gating is therefore **no longer a one-line basis toggle** — it needs the UI branches rebuilt, not just uncommented — but it already needed a full policy discussion per Hard Rule #1 regardless, and the removed code is small (a second bar trace, a caption, a threshold-denominator swap) and fully described in commit `06455d5`'s message if ever needed again.
...
**Optional columns beyond the original `CREATE TABLE` above** (each additive, each dropped-and-retried by `save_recommendations` until its DDL is applied — see `db.py`'s header docstring for the exact `ALTER TABLE` statements): `s_score`, `avg_sent` (sentiment, feeds the composite); added 2026-08-01 for F-225 Portfolio Q&A, `t_score`, `bq_score`, `val_score` (the 4-pillar breakdown, forward-only — rows saved before this date have these `NULL`); added 2026-08-23 for **F-249 Phase 2 sizing capture**, `rec_shares`, `rec_stop`, `rec_portfolio_value` (all `NUMERIC`) plus `rec_sizing_version` (`INTEGER`); added 2026-09-09 for **F-264 Watchlist ENTER_NOW capture**, `already_held` (`BOOLEAN`) — nullable, `NULL` for every existing row and every other `rec_type`, records whether an `enter_now` row's ticker was already held at capture time (fresh entry vs. add-to-existing provenance only; never changes how the row is graded); and added 2026-09-23 for **Phase 4 of the data-foundation strategy**, `weights_version` (`INTEGER`).

**`weights_version` — versions `COMPOSITE_WEIGHTS`' own weight values, not `rec_sizing_version`'s sizing formula.** Stamped unconditionally from `constants.COMPOSITE_WEIGHTS_VERSION` at the shared write boundary inside `save_recommendations` — never read from the caller's dict, so every existing build site gets it with zero changes required. `NULL` on a row means **"not-yet-backfilled"** (a transient state that should only exist between this deploy and the owner running the one-time backfill `UPDATE` below), **never** "unknown regime" — a reader encountering `NULL` should fall back to `rec_date < '2026-07-09'` as the regime signal, mirroring how `rec_sizing_version`'s own `NULL` semantics are documented above. Forward-only capture at the DDL boundary; the historical backfill is a separate one-time statement the owner runs by hand once the column exists:
...
- **Block E — Engine vs Analyst Calibration (Phase 3, SHIPPED 2026-08-29):** a 2×2 disagreement matrix — analyst consensus side (Buy/Sell, via `analyst_intel.consensus_side()`) × engine composite at save time (`>= COMPOSITE_BUY`). Hold/Mixed consensus rows are EXCLUDED from the matrix entirely (never forced onto the Buy/Sell axis), as are rows with a missing/NaN `composite_score_at_save`. Reuses Block B's SAME evaluable rows and the SAME `ANALYST_ACCURACY_DIRECTION_DAYS` (30) window and `directional_hit` verdict — a row cannot disagree with itself across blocks. Each cell shows case count + avg return; the two disagreement cells additionally show "engine right N / analyst right M" only once the cell reaches `ANALYST_CALIBRATION_MIN_CASES` (5). `analyst_intel.calibration_matrix()`, pure, 16 tests. Gated on `composite_score_at_save` populated on ≥20 rows — measured live 2026-08-29 (26 of 547), trigger met. **The original "selection bias" framing here is superseded** — the 2026-08-29 empty-cells reading was traced 2026-09-07 to a structural bug (the resolver only ever read `_port_df_enriched`, so a non-held ticker's composite always resolved to `None`), not mere skew, and was fixed the same day (`analyst_intel.trustworthy_composite()` + a Save-time fallback fetch). **Measured live 2026-09-12 after the fix: still only 23 of 442 evaluable rows classifiable** (383 lack a composite) — both disagreement cells remain below the `ANALYST_CALIBRATION_MIN_CASES` floor, because the fix is forward-only and only newly-saved rows carry the flag. The on-screen caption (derived live from `calibration_matrix()`'s own counts, never hardcoded) now calls this **dormant**, correcting an earlier version of the copy that called it a "skew" fading "as more research is saved" — at the current save rate that fade is years, not weeks.
- **Block F — 🪜 Consensus-Ladder Performance (added 2026-09-12, display-only, no new Phase number — extends Block A).** Tests whether `VALUATION_CONSENSUS_PTS`'s point spread (**compressed 2026-09-13, analyst-weight-audit Phase B, planner-designed, owner-approved: Strong Buy 15 / Buy 12 / Hold 8 / Mixed 5 / Sell 0; was 30/24/15/9/0** — 40 of the valuation pillar's 85 points when PT-upside and consensus are both present, i.e. ~14.1% of the whole composite (was 16.5%), **no longer rising to ~30% when `forward_pe`/`fcf_yield` are both absent — Phase B option (c) SHIPPED 2026-09-13, same day: `valuation_score()`'s `val_available` now requires at least one OBJECTIVE metric (Forward P/E or FCF Yield); analyst-only tickers WITHHOLD the valuation verdict (fabricated-neutral-50) instead of renormalising to 100% analyst opinion and reporting it available**) is actually earned — i.e. whether higher tiers produce better forward returns than lower ones. `analyst_intel.consensus_tier()` classifies a saved `consensus_rating` into all five tiers (matching the leading label only, same parenthetical-tally trap `consensus_side()` already guards against); `analyst_intel.ladder_performance()` reports `n`, avg `ret_pct`, and % positive per tier, gated on the same `ANALYST_CALIBRATION_MIN_CASES` floor as Block E. **Load-bearing design choice:** unlike Block E, this NEVER reads `directional_hit` — that field treats every non-bullish rating as pseudo-bearish (a Hold "hits" when the stock falls), so comparing tiers on it would compare "did Buy rise" against "did Hold fall," two different questions. It measures `ret_pct`, the same signed quantity for every tier. **First live measurement (2026-09-12):** the same 442-row Scorecard population showed 48% directional accuracy overall (95% CI ≈43.3–52.7% — no measurable directional edge, not "analysts are wrong"; the 48% is against a raw-return-vs-zero benchmark in a presumably bullish-skewed, drifting-up sample, so it is arguably worse than it reads). **First live reading of the ladder itself (2026-09-13):** Strong Buy n=395/45%/+1.8%, Buy n=0 (investigated and confirmed a genuine data-pattern fact — a single-firm article can only mathematically produce Strong Buy/Hold/Sell, never Buy/Mixed — not a bug), Hold n=38/53%/−0.8%, Sell n=22/64%/+4.2%: the return ordering is inverted at the extremes on real samples, sharper than the flat 48% alone. **SPY-relative alpha added 2026-09-13** — `ladder_performance()` gained an optional `spy_close_by_date` parameter; each tier now also reports `n_alpha`/`avg_alpha_pct`/`alpha_verdict_shown` (own independent floor, since a row's window can fall outside the fetched SPY history even when its raw `ret_pct` is known), rendered as the tile's `st.metric` delta. SPY history is fetched ONCE via the existing `_cached_spy("2y")` (matching Recommendations History's own `_rh_spy_by_date` pattern, `app.py` ~28686) rather than per-row, and 2y (not 6mo/1y) because saved `analyst_coverage` article dates can run over a year old. Full reasoning, the Buy=0 verification chain, and the coverage-asymmetry finding (PT upside duplicates yfinance's free `targetMeanPrice` fallback; consensus rating does not, because yfinance's `recommendationMean` is fetched every load and consumed by nothing): `docs/plans/analyst-weight-audit.md` §8a/§8b.
The two new columns are: `price_at_article_date` (populated at save-time via `_resolve_price_at_save()` in app.py, or backfilled by `scripts/backfill_analyst_prices.py` from yfinance) and `composite_score_at_save` (engine score at save, NULL for non-held tickers; feeds Block E above). Both added 2026-07-22. Rows with `no_anchor` status (missing price_at_article_date) now show a per-row "🔄 Fetch" button on the Research Scorecard tab (calls `analyst_intel.fetch_anchor_price()` live with yfinance→FMP failover, then saves via `db.update_analyst_coverage_price()`) instead of only being fixable via the periodic batch backfill.
...
Written by `db.save_thesis_erosion_cache()` (upsert, best-effort); read by `db.load_thesis_erosion_cache(ticker, score_date)`. **`_READONLY`-GATED as of 2026-08-28** (`bd42aa3`) — this doc previously said "System cache — not `_READONLY`-gated", a classification that propagated across four tables by cross-citation. It was wrong for all four: each write pays for a model call and persists model-authored PROSE the owner reads as content, which is not the harmless auto-warmed-cache class the exemption described. The real cost guard is at the app.py call site (the Haiku call precedes the write, so a write-layer no-op alone would re-bill on every rerun); the db-layer guard is defence-in-depth. Compute is gated on `is_trading_day()` to prevent weekend rows from corrupting the cross-day delta. RLS: `FOR ALL TO service_role`.
...
`db.load_regime_scenario_cache(scan_date)`. **`_READONLY`-GATED as of 2026-08-28** (`bd42aa3`) — this previously read
"Deliberately NOT `_READONLY`-gated ... a recomputable analytical narrative, not
user-authored data". Recomputable is true but is not the criterion: the write is
paid, and the prose lands on the owner's own decision surfaces, so a viewer would
be authoring content the owner reads (the contrast this
passage used to draw — with `price_xcheck_history`, "which IS gated" — was itself
stale and is corrected here: that writer was deliberately UN-gated on 2026-07-29
as an audit Medium finding, so the reconciliation this text claimed rested on a
distinction that no longer existed. `price_xcheck_history` remains ungated and
correctly so — no model call, and the upsert is idempotent on `(ticker,
check_date)` — but it is an example of the exempt class, not a contrast to it).
RLS: `FOR ALL TO service_role`.
...
### 6.44 `reference_tables` table

Corrected 2026-09-02 — the version previously here was reconstructed from guesswork rather than transcribed from the real DDL at `docs/sql/app_settings_ddl.sql`, and had 3 wrong details (see below). Exact DDL:
...
**Current state of hand-maintained reference rosters** (App Settings, F-262). One row per named roster (`sector_universe`, `discovery_universe`, `sector_candidates`) — `name` IS the primary key (not a surrogate `id`). `payload` is the full dict of tickers/metadata for that roster; `payload_hash` is the SHA256 hash (canonicalized via `reference_data.canonicalize`, so reordering is never mistaken for a change); `as_of` is the date the payload was last *meaningfully* changed (the "snooze-button" mechanism — a no-op save leaves `as_of` alone); `updated_by` is nullable. Written by `db.save_reference_table(name, payload, updated_by)` (upsert on `name`); read by `db.load_reference_table(name)`. Complements `reference_table_history` (6.45). Degrades to `None` on any load failure. RLS: `FOR ALL TO service_role`.

### 6.45 `reference_table_history` table

Corrected 2026-09-02, same reason as 6.44 above. Exact DDL:
...
**Append-only audit trail of reference-roster edits** (App Settings, F-262). **Correction: a row is inserted only when a save ACTUALLY CHANGES the canonicalized payload** — `db.save_reference_table()` (`db.py:5139-5141`) checks the new hash against the stored one and returns `{"status": "no_change"}` *before* either the `reference_tables` upsert or this history insert runs, so a no-op save writes nothing here (the previous version of this section claimed the opposite — that a row is inserted on every successful save regardless of change). Retention is keep-all by design (a JSON roster is a few KB, edits happen only a handful of times a year — `docs/plans/app-settings.md` §Q4); this is what lets a future session answer "who changed what, when" for a mutable universe the same way git history already does for `constants.py`. Queried by `db.load_reference_table_history(name, limit)` (returns `[]` if never edited, `None` on load failure). Degrades to `None` on any load failure. RLS: `FOR ALL TO service_role`.
...
Registered in `system_health.py`'s `_INVENTORY` (System Trust check ②) as a conditional,
existence-only store on the `premarket` lane, mirroring `analyst_target_snapshots` — added
2026-09-21, 8 days after ship, closing a dead-diagnostic gap this audit found (a silently
failed write or an un-applied DDL was previously invisible everywhere in the app).
...
5. GitHub Actions runs `tests.yml` (pytest over `stock_analyzer/` pure logic, 4368 tests as of 2026-08-27), `antipatterns-check.yml` (`no-new-antipatterns`) and `docs-check.yml` (`constants-documented`) on push, but **neither gates the deploy** — Railway and Streamlit Cloud don't consult GitHub Actions, so a red ❌ shows up on the commit, not a blocked deploy. Manual testing (post-deploy smoke check + feature click-through) remains the quality gate for anything outside `stock_analyzer/`'s pure logic — see `docs/testing-strategy.md` for the full split.

   **A repository ruleset requiring these three as push-blocking checks was tried and reverted the same day (2026-08-27).** Motivation: the local pre-commit hook's pytest gate fails OPEN (warns, does not block) when it cannot resolve `.venv` (`.claude/hooks/pre_tool_checks.py::_find_python`), which is how a red suite reached `main` silently for 4 days in the `77205a5` incident. GitHub's `required_status_checks` ruleset rule is built for a pull-request flow — it requires a commit SHA to already carry successful check-runs before that SHA can update a protected ref. Every workflow here triggers `on: push`, so a check can only run AFTER a commit lands on `main`; a brand-new direct-push commit can never satisfy "already checked" before it exists. Confirmed empirically: enabling it rejected every direct push with "3 of 3 required status checks are expected," including a completely green commit. **Do not re-enable this as Active enforcement without first switching to a branch+PR workflow** (the `pull_request:` trigger already exists on all three workflows and would let checks run before a merge, which is the model this mechanism actually supports) — re-trying it against direct pushes will reproduce the same lockout. If revisited, "Evaluate" enforcement (log without blocking) is the safe middle ground for a direct-push repo.
...
| Risk-off blindness made visible (F-271, 2026-09-14) | `risk_off_regime`'s documented "degrades to not-tripped on missing/short data" fail-safe used to render identically to a genuinely-measured calm market on the app's two UI consumers. New pure classifier `exit_advisor.risk_off_state(spy_trend_df, vix_level, *, trend_ma) -> "not_measured" \| "unusable" \| "measured"` (mirrors `util.factor_tilt_state`'s three-state shape — `measured` when the SPY close series has length `>= trend_ma` OR VIX is numeric, either leg alone sufficient; `not_measured` when both inputs are absent; `unusable` when at least one input is present but neither leg is usable). `market_risk_posture()` gained a `regime_state="measured"` keyword (backward-compatible default — every existing caller/test is unaffected) and its `regime_txt` no longer says "calm" unless the regime was actually measured; the returned dict carries the new `regime_state` field. Wired into exactly 2 sites: 🧾 Summary's "Risk Posture" tile (❔ icon + a single disclosure caption when not measured) and 🔗 Risk Analysis's "🧭 Market-Risk Posture" section (two distinct captions for `not_measured` vs `unusable`, and — the specific live bug this closes — the section's own `except Exception: _ro_armed, _ro_reasons = False, []` now also sets the state to `not_measured` instead of silently rendering "calm"). `risk_off_regime()` itself, and its three other callers (`daily_briefing.py`'s Home de-risk cards, `forward_sim.py`, `headless_alert_engine.py`'s premarket email), are unchanged — deliberately out of scope, since none of the three makes an affirmative "calm" claim today (they already fail silently, which is correct); extending disclosure there is a new feature, queued separately. | Live-confirmed in the app's own code, not a hypothetical: the Risk Analysis exception path was an instance of exactly the offline-sentinel-collapse class this project's own `check_antipatterns.py` gate exists to catch, just outside that gate's current AST patterns. **Redline, verified independently by both the `implementer` and the Opus `reviewer` via direct arithmetic trace:** `score`/`armed`/`risk_off` math is byte-for-byte unchanged — `regime_state` never enters that formula, so absent data still never arms a trim, suppression, or gate; this is display-only. Opus reviewer: SHIP, 0 blocking (two cosmetic non-blocking notes accepted as-is). No new constant — reuses `RISK_OFF_TREND_MA`/`RISK_OFF_VIX_LEVEL`. Full detail: `docs/requirements.md` F-271; roadmap `docs/plans/investor-maturity-roadmap.md` §5 C1. |
...
| Sizing suppression has FOUR causes, and they must never be conflated (F-249 2026-08-23; "capital" added by F-255 2026-08-25; "portfolio" added 2026-08-27) | `risk.sizing_unavailable_reason(portfolio_value, entry, stop, max_position_pct, net_capital, max_capital_pct)` is the **single** predicate for every no-size condition, returning `"stop"` (degenerate stop — a data problem the user can inspect), `"portfolio"` (the portfolio value is unknown or non-positive, so no honest size exists at any stop or any cap — the user fixes it by loading the portfolio, NOT by changing anything about the trade), `"ceiling"` (the 15% single-name cap can't afford one whole share — an account-size constraint no stop change fixes), `"capital"` (the SEPARATE 25% net-capital cap can't afford one share, or net capital is already ≤ 0), or `None`. `position_sizing` delegates **all** of its guards to it, the Grow Today adapter reads it instead of re-deriving any test, and the Analysis/Watchlist fallback captions branch on it. **Order is load-bearing:** `"stop"` precedes `"portfolio"` (a broken stop is worth reporting even when the book is unknown), and `"portfolio"` MUST precede `"ceiling"` because the ceiling test is itself gated on `portfolio_value > 0` and would leave the new branch dead if ordered after it. | Found by Opus review as blocking. `position_sizing` had gained a second `None` cause while the Analysis and Watchlist captions still said *"stop price too close to entry or not set"* — so a high-priced name on a modest book blamed the stop while a healthy 2×ATR stop rendered directly above it, on the surface whose whole job is explaining the suppression. One gate, one implementation: a second copy of either predicate drifts the moment `position_sizing`'s guards change. **This row itself proved the point on 2026-08-27:** it still said TWO after F-255 had made it three, i.e. the doc drifted exactly the way the code was designed not to. **The `"portfolio"` cause was a real defect, not a tidy-up** — `app.py` substituted `DEFAULT_PORTFOLIO_VALUE` (50,000) for an unknown book and sized real suggestions against it (~2× too many shares on a ~$24.5k account). Removing the substitution alone was NOT sufficient: with a zero book the predicate returned `None` ("I would size") and `max(1, int(0 / risk_per_share))` floored the result to **1 share**, so the fix had to land in the predicate. |
...
| Capital-equivalent risk disclosure (F-272, 2026-09-14) | `RISK_PCT_PER_TRADE` sizes every trade off the GROSS book inside `position_sizing()`, so at real leverage the disclosed risk-per-trade understates its true share of the owner's actual capital. New pure `risk.capital_equivalent_risk(risk_dollars, portfolio_value, net_capital, basis) -> dict` reframes the EXISTING risk figure already shown (`ps["actual_risk"]`/`ps["risk_pct_actual"]`) in capital terms — it invents no new number and changes no sizing math. Returns one of three explicit states — `{"state": "unlevered"}`, `{"state": "unknown"}`, or `{"state": "levered", "gross_pct", "capital_pct", "net_capital", "risk_dollars"}` — keyed **off `basis`** (`margin.resolve_net_capital`'s own basis string), never off `net_capital is None` alone, since `net_capital` is `None` in both the `"unlevered"` and `"stale"` cases and cannot distinguish them by itself; a `basis == "stale"` call resolves to `"unknown"` even if a stray non-`None` `net_capital` were passed alongside it. Wired into exactly 2 sites — 📈 Analysis's Detailed Analysis panel and 📋 Watchlist's ENTER_NOW panel — both reusing the `net_capital`/`basis` pair already resolved once per page render for the existing F-255 cap logic (`_f255_net_cap`/`_f255_basis` and their Watchlist-page siblings), never a second `resolve_net_capital` call. Renders **nothing** when unlevered (the existing gross-terms number already IS the capital-terms number — a "same as above" caption would be noise); an explicit "leverage unknown, update 💰 Account" caption when stale — never silence, never a fabricated number. | Never returns a bare `None` — a caller writing the natural `if result: render(...)` would silently skip exactly the "unknown" case that must speak, the same falsy-shortcut class this project has already shipped and fixed twice (`feedback_sentinel_is_present`, `feedback_none_sentinel_meets_pandas`). Deliberately scoped to 2 of the "five F-255 sizing surfaces": Grow Today, the pullback-add flow, and the cron/email buy-list only ever show position SIZE, never a risk-per-trade figure to reframe — extending a new risk metric there is a bigger, different change, queued separately (`docs/plans/investor-maturity-roadmap.md` §5, "Deferred, queued separately"). Opus-reviewed SHIP, 0 blocking. |
...
| Concentration-gate basis: EQUITY (current, 2026-07-09 — reverses the 2026-06-26 net-capital basis) | The single-name (15%) and sector (35%) hard gates, the `risk_advisor` `single_name_concentration` / `sector_concentration` trim recs, the Trade-Journal entry nudge, and the watchlist / quick-research / comparison entry-fit checks all gate on **plain equity weight** = MV ÷ invested equity. app.py sets it at ONE point (~app.py:4822 as of 2026-09-02, corrected from a stale ~2710 pointer that had drifted as the file grew): `Gate Weight (%)` := equity `Weight (%)`, `_acct_gate_cache.basis` := `"equity"`, and `gate_denom` (→ `risk_advisor`) := equity so `_acct_f` = 1. `daily_briefing` reads `_gate_wt`/`_gate_wt_col` (now == equity weight). `concentration.gating_denominator` is retired from the gate path (the fn remains, unused by gates). The margin-framing UI conditioned on `_acct_gate_cache.basis ∈ {account, over-levered}` **was REMOVED 2026-08-30** (F-260 findings #6/#9), not merely left inert as this row previously said — those branches had been unreachable since this 2026-07-09 reversal (the basis can never be anything but `"equity"`), so retaining them served no purpose and rendered a misleading "You carry a margin debit..." caption/second-bar-trace that could never actually appear. Leverage/margin risk is instead an **awareness-only** signal (`_leverage_cache` → 🔗 Risk Analysis leverage read + 💰 Account note; F-09d), never a gate. | The 2026-06-26 net-capital "tighter-of-both" basis made a **transient** margin balance lurch a hard gate — a sector read 166.9% and the app demanded ~85% liquidation off a temporary financing state. That fights §2B calm/anti-churn and conflates position-sizing (structural, judged on equity) with leverage (a distinct risk, better monitored than gated). Equity-basis is stable across transient margin and still fires on a genuinely over-concentrated book; leverage risk stays visible as advice. User decision 2026-07-09 (reverses the 2026-06-26 call). Do NOT re-base gates to net-capital without an explicit policy discussion — and note that doing so now also requires rebuilding the removed UI branches, not just flipping `gate_basis()`'s return value. |
...
| Watchlist "Ready to Enter" email (F-265, 2026-09-10) | Same `scan` cron run, after the buy-list block, in its own try/except so a fault here can't touch that email's send. `headless_alert_engine.compute_watchlist_entries` recomputes every watchlist ticker's verdict via the unchanged `watchlist_advisor.build_watchlist_recommendation`, filters to `action == "ENTER_NOW"` (the raw `qualifying` set — held and not-held alike). **Capture vs. announce are deliberately different scopes:** the FULL `qualifying` set is captured as `enter_now` recommendation rows (matching F-264's own capture scope exactly, `already_held` per ticker) so a cron-only day doesn't undercount held tickers in the grading dataset versus an interactive Watchlist visit — but only a narrower `eligible` subset (qualifying minus held tickers, minus today's scanner-Go picks, minus tickers under an active EXIT/TRIM/RISK_OFF protective call) is ever emailed. Emailed further gated on a **transition-only** cadence: only a ticker newly entering `eligible` versus the prior NYSE trading day's recorded `enter_now` set fires — a persisting name stays silent, a genuine drop-out-then-return re-fires. When the portfolio-beta leg can't be built headlessly, a qualifying name is still emailed with a disclosed "beta-fit check unavailable this run" note rather than silently withheld or silently included without the caveat. `notify.render_watchlist_entries_email` mirrors the buy-list email's style; card prose routes through the existing escape-then-bold `_email_md_inline` convention. New `alert_state` dedup row (saved only on a real send). | ENTER_NOW was previously discoverable only by manually opening 📋 Watchlist — no email, no Brief mention, no nav badge (unlike 🔔 Catalyst Watch's dynamic badge). This closes that gap using the transition-only cadence the app already applies elsewhere to avoid re-nagging (Thesis erosion jump-gating, the Structural Alert banner's "newly-formed since last scan" framing) rather than `new_pick`'s own no-dedup daily re-surfacing — a watchlist name is one the owner personally curated and is already choosing whether to act on, so a persisting call is a decision already made, not new information. The exclusions are the coordination: a scanner-Go ticker and a protective-call ticker are each already being surfaced/handled by a different existing channel, so this email would otherwise silently double-announce or contradict them. Design: two `planner` passes (base grading feature, then this surfacing feature); Opus reviewer SHIP, 0 blocking on both. |
...
| Partial unique index + ON CONFLICT (F-268 SnapTrade/income CSV upsert gotcha) | `snaptrade_income_events` table's unique constraint was created as a partial index (`WHERE snaptrade_txn_id IS NOT NULL`). PostgreSQL requires `ON CONFLICT (column)` upsert attempts to repeat the exact partial predicate to match; PostgREST's `on_conflict=` parameter does not automatically add the WHERE clause, so every upsert without the predicate throws `there is no unique or exclusion constraint matching the ON CONFLICT specification` and fails silently in the catch handler, producing false "already recorded"/"0 new rows" results. **Fix:** use a full (non-partial) unique index for on-conflict-targetable columns. Safe here because both `snaptrade_income_events` write paths (`db.save_snaptrade_income_events` and the new `db.save_income_events_from_csv`) already filter out NULL-`snaptrade_txn_id` rows before upserting; pre-existing NULL rows (seed data, if any) must be deleted separately. | Partial indexes optimize storage for sparse columns but incur a schema complexity tax: the uniqueness constraint becomes "silent unless the exact SQL recipe is replicated" rather than "use the constraint name". A future schema change to a partial-indexed column carries forward this exact same risk class. When writing a new upsert path for a partial-indexed column, check whether the new path needs to repeat the predicate in its `on_conflict=` clause (test with sample NULL and non-NULL rows both present). |
