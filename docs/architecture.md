...
| Catalyst Watch (awareness, not a gate) | A panel in the Pre-Market Intel area (ungated by `is_premarket()` so after-close reports show intraday) lists upcoming earnings (≤ `CATALYST_WATCH_WINDOW_DAYS`) for names the app tracks (held ∪ watchlist ∪ sector universe), tagged with sector, a 🔥 leading-sector flag, days-until and held/watchlist/universe. `catalyst_watch.build_catalyst_watch` (pure) intersects a market-wide earnings calendar (`data.fetch_earnings_calendar` → FMP, 24h cached) with the tracked set; on failure it degrades to held-only earnings. | The app intentionally won't RECOMMEND buying into earnings (binary risk), but it was BLIND to a tracked name reporting (PANW beat after close, nothing surfaced). Awareness ≠ recommendation: inform on the research surface, leave the call to the user; confirmed post-print moves still surface via Movers. (Commit dd8aea5.) |
