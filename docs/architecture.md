...
5. GitHub Actions runs `tests.yml` (pytest over `stock_analyzer/` pure logic, 4368 tests as of 2026-08-27), `antipatterns-check.yml` (`no-new-antipatterns`) and `docs-check.yml` (`constants-documented`) on push. **A GitHub repository ruleset on `main`** (added 2026-08-27, `enforcement: active`, empty bypass list) makes all three **required status checks** — a push is expected to be rejected if any of the three does not report success for that commit; this was added specifically to close the gap where the local pre-commit hook's pytest gate fails OPEN (warns, does not block) when it cannot resolve `.venv` (see `.claude/hooks/pre_tool_checks.py::_find_python`), which let a red suite reach `main` silently for 4 days in the `77205a5` incident. **This still does not gate the Railway/Streamlit deploy directly** — those platforms don't consult GitHub at all, so once a push is accepted it auto-deploys regardless. Manual testing (post-deploy smoke check + feature click-through) remains the quality gate for anything outside `stock_analyzer/`'s pure logic — see `docs/testing-strategy.md` for the full split.
