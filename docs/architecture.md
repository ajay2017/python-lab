...
| `REFERENCE_SHELF_LIFE_DAYS` | `{sector_universe: 90, discovery_universe: 180, sp500_sector_weights: 180, sector_candidates: 180}` | How many days a hand-maintained static reference table stays trustworthy before 🩺 System Trust check ⑤ flags it for a human refresh. Keyed to the registry in `stock_analyzer/reference_shelf.py` (a test asserts both key sets match in both directions). `sector_universe` is tightest at 90d because it's the ~70-name net Grow Today scans *daily*; the rest are semi-annual. **Observability only** — never gates a recommendation, suppresses a pick, or changes a score. |
...
| `ALPHA_ATTRIBUTION_MIN_SNAPSHOT_DAYS` | 180 | AI Insights → Alpha Attribution panel (app.py) — daily-snapshot history needed before the factor-attribution decomposition activates. Display/activation-gate only, feature not yet active. Was 8 bare literal copies of "180" (2026-08-04 audit finding) |
