...
| Concentration-gate basis: "tighter-of-both" (margin-aware) | The single-name (15%) and sector (35%) hard gates compare a **`Gate Weight (%)`** column — not the equity-only display `Weight (%)` — against the ceilings. `gating_denominator(equity, account_total, stale)` (pure, in `concentration.py`) returns `gate_denom = min(equity, total_account_value)` where `total_account_value = equity + signed net cash` (negative = margin debit): **margin tightens** the gate (smaller denominator → higher weights → ceilings bite sooner), **cash never loosens** it (equity denominator kept), unknown/stale cash (> `ACCOUNT_CASH_STALE_DAYS`=7) **falls back to equity-basis**, and net capital ≤ 0 is "over-levered" → suppress all adds. app.py loads `db.load_account_cash()` once, computes the denom, injects the `Gate Weight (%)` column on `port_df` at the boundary (before the brief builds), and publishes `_acct_gate_cache` {denom, basis, over_levered}. `daily_briefing` reads it via `_gate_wt`/`_gate_wt_col` (fallback to `Weight (%)` so non-app callers / the headless cron are unaffected). The entry nudge (Trade Journal) uses the published denom; a "⚖️ Tightened by margin" annotation renders on the suppression banners + nudge + trim recs when account-basis is active. **Phase 1 = hard gates + entry nudge. Phase 2 = the `risk_advisor` `single_name_concentration` / `sector_concentration` trim recs (passed `gate_denom`; weight scaled by `equity/gate_denom`, gate + displayed weight + trim pp/$ all re-based together, "$ at risk" unchanged = market value) + the watchlist / quick-research / comparison entry-fit sector checks (sum the `Gate Weight (%)` column).** Beta/Sharpe recs keep equity weight (different risk dimension). The watchlist entry-fit was also repointed from the raw `holdings_df` (no `Sector` column → the sector check had never fired) to the enriched `_port_df_enriched`, activating that sector caution for the first time. | The equity-only gate was blind to leverage — a margined-up single name looked identical in % to an unmargined one, yet a drawdown destroys far more of real capital. Net-capital basis is the institutional standard (limits measured against NAV); the asymmetric "cash never loosens" choice fits the app's protective posture and the manually-seeded/possibly-stale cash figure (only the *sign* is trusted to tighten, never the magnitude to loosen). Display weights stay equity-basis to avoid confusing the at-a-glance read. User decision 2026-06-26 (closes the deferred gate-basis question from the account-baseline cluster). |
