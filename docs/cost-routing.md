...
| 2026-06-23 | Concentration/sizing discipline — build (Part 1 entry nudge + Part 2a single-name MEDIUM risk rec + Part 2b high-beta standing line) | — | — | — | — | n/a — lead | Build = inline lead (Opus). Pure module + risk-advisor wiring; unit-tested `assess_add_concentration` + `high_beta_share`. Review = Opus reviewer (mandatory, touched trade-entry + new rec type + new constant); caught NameError-class blocker (`_f` defs, py_compile gap), fixed + re-confirmed SHIP. Docs = Haiku (this row). |
